...
class NoteService(BaseService[Note, NoteCreate, NoteUpdate]):
    ...
    async def create_note(self, db: Session, note_create: NoteCreate, user_id: UUID) -> Note:
        """
    ...
    async def get_note(self, db: Session, note_id: UUID, user_id: UUID) -> Optional[Note]:
        """
    ...
    async def search_notes(
        ...
        try:
            # Build base query
            query_obj = db.query(Note).filter(Note.user_id == user_id)
    ...
    async def bulk_delete_notes(
        ...
        try:
            ...
            db.rollback()
            logger.error(f"Error in bulk_delete_notes: {str(e)}")
    ...
    async def update_note(
        ...
        Returns:
            Updated note if successful, None if not found
        """
        note = await self.get_note(self.db, note_id, user_id)
        if not note:
            return None
